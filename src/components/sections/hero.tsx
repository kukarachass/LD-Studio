...
const TAGS = [
  { label: "BI-LED", tone: "border-magenta/45 text-magenta" },
  { label: "RGB", tone: "border-violet/45 text-violet" },
  { label: "AMBIENT", tone: "border-cyan/45 text-cyan" },
  { label: "КАСТОМ", tone: "border-violet/45 text-violet" },
];
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-y-0 right-0 w-full lg:w-[56%]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,3,8,0.9)_0%,rgba(5,3,8,0.74)_45%,rgba(5,3,8,0.97)_100%)] lg:bg-[linear-gradient(90deg,var(--color-void)_2%,rgba(5,3,8,0.88)_30%,rgba(5,3,8,0.2)_74%,rgba(5,3,8,0.5)_100%)]" />
      </div>
      ...
      <div className="section-x relative mx-auto w-full max-w-[110rem]">
        <ul className="animate-fade-up flex flex-wrap gap-2">
          {TAGS.map((tag) => (
            <li
              key={tag.label}
              className={`rounded-full border bg-void/40 px-3 py-1.5 font-mono text-[10px] tracking-[0.18em] backdrop-blur-sm ${tag.tone}`}
            >
              [{tag.label}]
            </li>
          ))}
